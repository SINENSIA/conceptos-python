...
def buscar_pelicula(peliculas):
    ...
    print('----------------------------')
    pelicula = input(
        COLOR_VERDE + "Nombre de la película a buscar: " + RESET_COLOR)
    if validar(pelicula):
        pattern = re.compile(pelicula, re.IGNORECASE)
        for nombre, detalles in peliculas.items():
            if pattern.search(nombre):
                print(
                    f"Encontrada: {nombre} - Director: {detalles['director']}, Año: {detalles['año']}, Presupuesto: {detalles['presupuesto']} millones")
                print('----------------------------')
                print(COLOR_VERDE + "La Película " + COLOR_AZUL + pelicula + RESET_COLOR +
                      COLOR_VERDE + " se ha encontrado en la lista." + RESET_COLOR)
                break
